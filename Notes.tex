...
semantics does it by attaching mathematical meanings to its terms)
    ...
    \item \textbf{Decent}:
     a term t is said to be decent in the calcu- lus λZ if every subterm v appearing in some substituted subterm u[x/v] of t is λZ- strongly normalising. As an example, the term x[x/(y y)][y/λw.w w] is decent in λes since y y and λw.w w are both λes-strongly normalising, but its Comp2-reduct x[x/(y y)[y/λw.w w]] is not. 
